...
- **家长端**(iPhone/Android):按 语文/数学/英语/其他 分组展示**任意日期**(前一天/后一天 + 最近 7 天快捷条)、**文字录入为主**、语音说话自动转文字(辅助)、分组配置(配置一次长期生效)、编辑/删除
...
5. 家长端/儿童端都可以用「‹ 前一天 / 后一天 ›」或顶部日期条回看历史作业(不能翻到未来)
6. 孩子在 iPad 上看今天作业、点语音回放、回看之前日期(不能看未来,不能修改)
7. 换设备/换身份:设置页可改服务器、换房间、切换家长↔儿童
